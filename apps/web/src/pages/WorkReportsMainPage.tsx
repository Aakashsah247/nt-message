...
import {
  ...
  getWorkReportSnapshot,
  getWorkReportSummary,
  listWorkReportSnapshots,
  saveWorkReportSnapshot,
} from "../services/work-main-parity.service";
import type {
  WorkReportQuery,
  WorkReportSavedSnapshot,
  WorkReportSnapshotSummary,
} from "../services/work-main-parity.service";
import type {
...
type SavedReportEnvelope = {
  period?: { from?: string | null; to?: string | null };
  content?: unknown;
};

type SavedOverviewContent = Pick<
  WorkReportSummary,
  "work" | "workflow" | "teams"
>;

interface SavedPerformanceGroup {
  id: string;
  name: string;
}

interface SavedPerformanceReference {
  display: string;
}

interface SavedPerformanceRow {
  date: string;
  operationalTeam?: { id: string; name: string } | null;
  orgUnit?: { id: string; name: string } | null;
  references?: SavedPerformanceReference[];
  workTypes?: Record<string, WorkReportPerformanceCounts>;
}

interface SavedPerformanceContent {
  workTypeGroups?: SavedPerformanceGroup[];
  rows?: SavedPerformanceRow[];
}

interface SavedWorkRecordRow {
  id: string;
  ticketNumber: string;
  workType?: { name: string } | null;
  executionTeams?: Array<{ name: string }>;
  primaryAssignee?: string | null;
  reference?: { display: string } | null;
  workflowStage: string;
  createdAt: string;
}

interface SavedDutyContent {
  dutySummary?: WorkReportDrilldownResponse["dutySummary"];
  sections?: {
    duty?: { rows?: WorkReportDrilldownDutyRow[] } | null;
  };
  notice?: string;
}

function savedReportEnvelope(value: unknown): SavedReportEnvelope {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  return value as SavedReportEnvelope;
}
...
function scopeTitle(role: string | undefined): string {
  if (role === "SUPER_ADMIN") return "Office Reports";
...
function workflowStageFilterLabel(
...
function ReportSkeleton({
  ...
  return (
    <section
      className={`report-v2-skeleton report-v2-skeleton--${variant}`}
      role="status"
      aria-live="polite"
      aria-label={label}
    >
...
function OverviewReport({
  ...
  if (!summary && loading) {
    return (
      <ReportSkeleton
        variant="overview"
        label="Preparing the report overview"
      />
    );
  }
  ...
  const workflow = [
    {
      key: "new",
      label: "New",
      value: summary.workflow.newWork,
      tone: "neutral",
    },
    {
      key: "progress",
      label: "In progress",
      value: summary.workflow.inProgress,
      tone: "blue",
    },
    {
      key: "sales",
      label: "Waiting for Sales",
      value: summary.workflow.waitingForSales,
      tone: "amber",
    },
    {
      key: "approval",
    ...
    },
    {
      ...
      value: summary.workflow.returnedForCorrection,
      tone: "purple",
    },
  ];
  const workflowMax = Math.max(1, ...workflow.map((item) => item.value));

  const trendCreatedTotal = summary.trend.reduce(
    (total, day) => total + day.workCreated,
    0,
  );
  const trendCompletedTotal = summary.trend.reduce(
    (total, day) => total + day.workClosed,
    0,
  );
  ...
  const trendBuckets = useWeeklyTrend
    ? Array.from(
        { length: Math.ceil(summary.trend.length / 7) },
        (_, bucketIndex) => {
          const days = summary.trend.slice(
            bucketIndex * 7,
            bucketIndex * 7 + 7,
          );
  ...
  const orderedTeams = [...summary.teams].sort((left, right) => {
    if (right.overdueWork !== left.overdueWork)
      return right.overdueWork - left.overdueWork;
  ...
  const attention: Array<{
    ...
    },
    {
      ...
      value: summary.workflow.waitingForSales,
      tone: "amber",
      note: "Primary team is waiting on Sales",
      stage: "WAITING_FOR_SALES",
    },
    {
      label: "Waiting for Approval",
      value: summary.workflow.waitingForApproval,
      tone: "amber",
      note: "Manager review is required",
    ...
    },
    {
      ...
      value: summary.workflow.returnedForCorrection,
      tone: "purple",
      note: "Worker action is required",
      ...
      <div className="report-v2-main-grid">
        <section className="report-v2-panel report-v2-panel--workflow">
          <header className="report-v2-panel__header">
            ...
            <small>
              {summary.period.days} day{summary.period.days === 1 ? "" : "s"}
            </small>
          </header>
          <div className="report-v2-workflow-list">
            {workflow.map((item) => (
              <div
                key={item.key}
                className={`report-v2-workflow-row is-${item.tone}`}
              >
                <div>
                  <span>{item.label}</span>
                  <strong>{item.value}</strong>
                ...
                <div className="report-v2-workflow-track" aria-hidden="true">
                  <i
                    style={{
                      width: `${item.value === 0 ? 0 : Math.max(4, (item.value / workflowMax) * 100)}%`,
                    }}
                  />
                </div>
              </div>
      ...
      <section className="report-v2-panel report-v2-panel--teams">
        <header className="report-v2-panel__header">
          <div>
            ...
            <p>
              Team ownership is shown first; individual participation stays in
              work details.
            </p>
          </div>
          <small>
        ...
        {orderedTeams.length === 0 ? (
          ...
          <div
            className="report-v2-team-table-wrap"
            tabIndex={0}
            aria-label="Team work performance table"
          >
                  ...
                  <tr key={team.teamId}>
                    <td>
                      ...
                      >
                        ...
                        <small>
                          {team.departmentName} · {team.divisionName}
                        </small>
                      </button>
                    </td>
                    ...
                    <td
                      className={team.overdueWork > 0 ? "is-alert" : undefined}
                    >
                      {team.overdueWork}
                    </td>
      ...
      <section className="report-v2-panel report-v2-panel--trend">
        <header className="report-v2-panel__header">
          <div>
            ...
            <p>
              Compare incoming work with completed work. Every bar shows the
              exact count.
            </p>
          </div>
        ...
        {summary.trend.length === 0 ? (
          <div className="report-v2-empty">
            No work was created or completed in this period.
          </div>
          ...
          >
            ...
            {[
              {
                label: "Created",
                value: singleTrendDay.workCreated,
                tone: "created",
              },
              {
                label: "Completed",
                value: singleTrendDay.workClosed,
                tone: "completed",
              },
            ].map((item) => (
              <div
                key={item.label}
                className={`report-v2-trend-comparison__row is-${item.tone}`}
              >
                <div>
                  <span>{item.label}</span>
                  <strong>{item.value}</strong>
                ...
                <div
                  className="report-v2-trend-comparison__track"
                  aria-hidden="true"
                >
                  <i
                    style={{
          ...
          <>
            <div
              className="report-v2-trend-totals"
              aria-label="Work trend period totals"
            >
              <span>
                <small>Created</small>
                <strong>{trendCreatedTotal}</strong>
              </span>
              <span>
                <small>Completed</small>
                <strong>{trendCompletedTotal}</strong>
              </span>
            </div>
            <div className="report-v2-bar-chart__heading">
              <strong>
                {useWeeklyTrend ? "Weekly comparison" : "Daily comparison"}
              </strong>
              <span>
                {useWeeklyTrend
            ...
            >
              <div
                ...
                style={{
                  minWidth: `${Math.max(520, trendBuckets.length * 82)}px`,
                }}
              >
                  ...
                  >
                    <div className="report-v2-bar-chart__bars">
                      {[
                        ...
                        <div
                          key={item.label}
                          className={`report-v2-bar-chart__bar is-${item.tone}`}
                        >
                          <strong>{item.value}</strong>
                          <span
                            className="report-v2-bar-chart__track"
                            aria-hidden="true"
                          >
                            <i
                              style={{
                                height: `${(item.value / trendBucketMax) * 100}%`,
                              }}
                            />
                          </span>
                        </div>
                    ...
                    <span className="report-v2-bar-chart__label">
                      {bucket.label}
                    </span>
                  </div>
                ))}
...
function WorkRecordsView({
  ...
  if (!report && loading)
    return <ReportSkeleton variant="records" label="Preparing work records" />;
  if (!section)
    return (
      <section className="report-v2-state">
        <strong>Work records are unavailable.</strong>
      </section>
    );
  ...
  return (
    <>
      <section className="report-v2-records-panel" aria-label="Work records">
        ...
        {rows.length === 0 ? (
          <div className="report-v2-empty report-v2-records-empty">
            No work records match the selected filters.
          </div>
        ) : (
          <>
            <div
              className="report-v2-records-head is-compact"
              aria-hidden="true"
            >
              <span>Ticket</span>
              <span>Work Type</span>
            ...
            <div className="report-v2-records-list">
              {rows.map((row) => {
                ...
                const workflowClass = row.workflowStage
                  .toLowerCase()
                  .replaceAll("_", "-");
                return (
                  <article
                    key={row.id}
                    className={`report-v2-record is-${workflowClass}`}
                  >
                    <div className="report-v2-record__row">
                      <div className="report-v2-record__ticket">
                        <strong>{row.ticketNumber}</strong>
                      </div>
                      <div className="report-v2-record__work">
                        <strong>{row.workType.name}</strong>
                      </div>
                      <div className="report-v2-record__owner">
                        <strong>{owner || "Not assigned"}</strong>
                      </div>
                      <div className="report-v2-record__reference">
                        <strong>{referenceLabel(row)}</strong>
                      </div>
                      <div className="report-v2-record__stage">
                        <span className={`report-v2-stage is-${workflowClass}`}>
                          {recordStageLabel(row.workflowStage)}
                        </span>
                      </div>
                      <div className="report-v2-record__date">
                        <strong>{formatDate(row.createdAt)}</strong>
                      </div>
                      <button
                        type="button"
                        className="report-v2-record__view-button"
                        onClick={() => setSelectedId(row.id)}
                        aria-label={`View details for ${row.ticketNumber}`}
                      >
            ...
            {pagination.totalPages > 1 && (
              <footer
                className="report-v2-pagination"
                aria-label="Work records pages"
              >
                <button
                  type="button"
      ...
      {selectedRow && (
        <div className="report-v2-record-drawer-layer">
          <button
            type="button"
            className="report-v2-record-drawer-backdrop"
            aria-label="Close work record details"
            onClick={() => setSelectedId(null)}
          />
          <aside
            className="report-v2-record-drawer"
            role="dialog"
            aria-modal="true"
            aria-labelledby="work-record-drawer-title"
          >
            <header className="report-v2-record-drawer__header">
              <div>
                <span>Work record</span>
                <h2 id="work-record-drawer-title">
                  {selectedRow.ticketNumber}
                </h2>
                <p>{selectedRow.workType.name}</p>
              </div>
              <button
                type="button"
                onClick={() => setSelectedId(null)}
            ...
            <div className="report-v2-record-drawer__body">
              <section className="report-v2-record-drawer__summary">
                <div>
                  <span>Stage</span>
                  <strong>{recordStageLabel(selectedRow.workflowStage)}</strong>
                </div>
                <div>
                  <span>Date</span>
                ...
                <div>
                  <span>Team / Owner</span>
                  <strong>
                    {selectedRow.assignedTeam?.name ??
                      selectedRow.primaryAssignee}
                  </strong>
                </div>
                <div>
                  <span>Due date</span>
                  <strong>{formatDate(selectedRow.dueAt)}</strong>
                </div>
              </section>
              <section className="report-v2-record-drawer__section">
                ...
                <dl className="report-v2-record-drawer__facts">
                  <div>
                    <dt>Reference</dt>
                    <dd>{referenceLabel(selectedRow)}</dd>
                  </div>
                  {selectedRow.information.map((item) => (
                    <div
                      key={`${selectedRow.id}:${item.code}`}
                      className={
                        selectedRow.information.length === 1
                          ? "is-wide"
                          : undefined
                      }
                    >
              ...
              <section className="report-v2-record-drawer__section">
                ...
                <dl className="report-v2-record-drawer__facts">
                  <div>
                    ...
                    <dd>{selectedRow.responsibleManager}</dd>
                  </div>
                  <div>
                    <dt>Started By</dt>
                    <dd>{selectedRow.startedBy ?? "Not started"}</dd>
                  </div>
                  <div>
                    <dt>Sales Staff</dt>
                    <dd>{selectedRow.salesMember ?? "Not required"}</dd>
                  </div>
                  <div>
                    <dt>Sales Status</dt>
                    <dd>{salesStatusLabel(selectedRow)}</dd>
                  </div>
                  {selectedRow.supportingStaff.length > 0 && (
                    <div className="is-wide">
                      <dt>Supporting Staff</dt>
                      <dd>{selectedRow.supportingStaff.join(", ")}</dd>
                    </div>
                  )}
                </dl>
              </section>
              {selectedRow.closedAt && (
                <section className="report-v2-record-drawer__section">
                  <h3>Completion</h3>
                  <dl className="report-v2-record-drawer__facts">
                    <div>
                      <dt>Manager Approved</dt>
                      <dd>{formatDateTime(selectedRow.closedAt)}</dd>
                    </div>
                  </dl>
                </section>
              )}
              {selectedRow.childProgress.total > 0 && (
                <section className="report-v2-record-drawer__section">
                  <h3>Delegation</h3>
                  <div className="report-v2-record__progress">
                    <div>
                      <span>Progress</span>
                      <strong>
                        {selectedRow.childProgress.completed} of{" "}
                        {selectedRow.childProgress.total} completed
                      </strong>
                    </div>
                    <div
                      className="report-v2-record__progress-track"
                      aria-hidden="true"
                    >
                      <i
                        style={{
                          width: `${selectedRow.childProgress.percentage ?? 0}%`,
                        }}
                      />
                    </div>
                  </div>
...
function PerformanceReportView({
  ...
  if (!report && loading) {
    return (
      <ReportSkeleton
        variant="records"
        label="Preparing the performance report"
      />
    );
  }
  ...
  if (!report || !section) {
    return (
      <section className="report-v2-state">
        ...
        <p>
          Refresh the report or change the selected period and organization
          filters.
        </p>
      </section>
    );
  ...
  const visibleGroups = workTypeId
    ? section.workTypeGroups.filter(
        (group) => group.definitionId === workTypeId,
      )
    : section.workTypeGroups;
  const showOverallTotal = visibleGroups.length > 1;

  return (
    <div className="report-v2-performance-view">
      <section
        className="report-v2-performance-panel"
        aria-label="Work performance report"
      >
        ...
        {rows.length === 0 ? (
          <div className="report-v2-empty">
            No operational Team work matches the selected period and
            organization filters.
          </div>
        ) : (
          <>
            <div
              className="report-v2-performance-table-wrap"
              tabIndex={0}
              aria-label="Work performance report table"
            >
              <table
                className={`report-v2-performance-table${showOverallTotal ? "" : " is-single-type"}`}
              >
                <caption className="report-v2-visually-hidden">
                ...
                <tbody>
                  {rows.map((row: WorkReportPerformanceRow, index) => (
                    <tr key={`${row.date}:${row.team.id}`}>
                      ...
                      <td
                        data-label="Date"
                        className="report-v2-performance-date"
                      >
                        {formatDate(row.date)}
                      </td>
                      <td
                        data-label="Team"
                        className="report-v2-performance-team"
                      >
                        <strong>{row.team.name}</strong>
                        <small>
                          {row.team.departmentName} · {row.team.divisionName}
                        </small>
                      </td>
                      <td data-label="Support Staff">
                        {row.supportStaffCount}
                      </td>
                      <td data-label="Other Staff">{row.otherStaffCount}</td>
                      <td
                        data-label="Reference"
                        className="report-v2-performance-references"
                      >
                      ...
                      {visibleGroups.map((group) => (
                        <Fragment key={group.id}>
                          <td data-label={`${group.name} tickets`}>
                            {
                              (
                                row.workTypes[group.id] ?? {
                                  tickets: 0,
                                  completed: 0,
                                  pending: 0,
                                }
                          ...
                          <td data-label={`${group.name} completed`}>
                            {
                              (
                                row.workTypes[group.id] ?? {
                                  tickets: 0,
                          ...
                          <td data-label={`${group.name} pending`}>
                            {
                              (
                                row.workTypes[group.id] ?? {
                                  tickets: 0,
                      ...
                      {showOverallTotal && (
                        <>
                          <td
                            ...
                            className="report-v2-performance-total"
                          >
                            {row.total.tickets}
                          </td>
                          <td
                            data-label="Total completed"
                            className="report-v2-performance-total is-completed"
                          >
                            {row.total.completed}
                          </td>
                          <td
                            data-label="Total pending"
                            className="report-v2-performance-total is-pending"
                          >
                            {row.total.pending}
                          </td>
                        </>
                ...
                <tfoot>
                  <tr>
                    ...
                    {visibleGroups.map((group) => (
                      <Fragment key={group.id}>
                        {performanceCountsCells(
                          totals.workTypes[group.id] ?? {
                            tickets: 0,
                            completed: 0,
                            pending: 0,
                          },
            ...
            <div
              className="report-v2-performance-cards"
              aria-label="Performance report mobile view"
            >
              {rows.map((row: WorkReportPerformanceRow, index) => (
                <article
                  key={`${row.date}:${row.team.id}`}
                  className="report-v2-performance-card"
                >
                  <header>
                    <span>#{index + 1}</span>
                    <div>
                      <strong>{row.team.name}</strong>
                      <small>
                        {formatDate(row.date)} · {row.team.departmentName}
                      </small>
                    </div>
                  </header>
                  <dl>
                    <div>
                      <dt>Support Staff</dt>
                      <dd>{row.supportStaffCount}</dd>
                    </div>
                    <div>
                      <dt>Other Staff</dt>
                      <dd>{row.otherStaffCount}</dd>
                    </div>
                    ...
                    {visibleGroups
                      ...
                      .map((group) => {
                        ...
                        return (
                          <div key={group.id} className="is-group">
                            <dt>{group.name}</dt>
                            <dd>
                              Tickets {counts.tickets} · Completed{" "}
                              {counts.completed} · Pending {counts.pending}
                            </dd>
                          </div>
                        );
                      })}
                    {showOverallTotal && (
                      <div className="is-group report-v2-performance-card__total">
                        <dt>Total</dt>
                        <dd>
                          Tickets {row.total.tickets} · Completed{" "}
                          {row.total.completed} · Pending {row.total.pending}
                        </dd>
                      </div>
                    )}
                  </dl>
      ...
      <section className="report-v2-duty-panel">
        <header className="report-v2-duty-panel__header">
          <div>
            ...
            <p>
              One concise schedule view. Duty records represent planned
              assignments, not attendance.
            </p>
          </div>
        ...
        {rows.length === 0 ? (
          ...
          <div
            className="report-v2-duty-table-wrap"
            tabIndex={0}
            aria-label="Duty schedule table"
          >
            <table className="report-v2-duty-table">
              <caption className="report-v2-visually-hidden">
                Planned duty schedule
              </caption>
              <thead>
                <tr>
                  <th>Date</th>
                ...
                {rows.map((row: WorkReportDrilldownDutyRow) => {
                  ...
                  return (
                    <tr key={row.id}>
                      ...
                      <td
                        data-label="Employee"
                        className="report-v2-duty-person"
                      >
                      ...
                      <td data-label="Shift">
                        ...
                        <small>
                          {formatDutyTime(row.startsAt, row.endsAt)}
                        </small>
                      </td>
                      <td data-label="Org Unit">
                        <strong>{row.orgUnit?.name ?? "Office-wide"}</strong>
                        {row.orgUnit?.code ? (
                          <small>{row.orgUnit.code}</small>
                        ) : null}
                      </td>
                      <td data-label="Operational Team">
                        <strong>{row.operationalTeam?.name ?? "—"}</strong>
                      </td>
                      <td data-label="Supervisor">
                        <strong>{row.supervisor || "—"}</strong>
                        {row.supervisorEmployeeId ? (
                          <small>{row.supervisorEmployeeId}</small>
                        ) : null}
                      </td>
                      ...
                      <td data-label="Status">
                        <span
                          className={`report-v2-duty-status ${status === "Cancelled" ? "is-cancelled" : "is-scheduled"}`}
                        >
                          {status}
                        </span>
                        {status === "Cancelled" && row.cancellationReason && (
...
function SavedReportSnapshotView({
  snapshot,
  onClose,
}: {
  snapshot: WorkReportSavedSnapshot;
  onClose: () => void;
}) {
  const payload = savedReportEnvelope(snapshot.payload);
  ...
  const overviewContent =
    ...
    !Array.isArray(payload.content)
      ? (payload.content as SavedOverviewContent)
      : null;
  const performanceContent =
    snapshot.dataset === "TECHNICAL_PERFORMANCE" &&
    payload.content &&
    typeof payload.content === "object" &&
    !Array.isArray(payload.content)
      ? (payload.content as SavedPerformanceContent)
      : null;
  const workRecordRows =
    snapshot.dataset === "WORK_RECORDS" && Array.isArray(payload.content)
      ? (payload.content as SavedWorkRecordRow[])
      : null;
  const dutyContent =
    snapshot.dataset === "DUTY_ASSIGNMENTS" &&
    payload.content &&
    typeof payload.content === "object" &&
    !Array.isArray(payload.content)
      ? (payload.content as SavedDutyContent)
      : null;

  return (
    <section
      className="report-v2-performance-panel"
      aria-label="Saved report snapshot"
    >
      <header className="report-v2-performance-panel__header">
        <span>Saved report · immutable snapshot</span>
        <h2>{snapshot.name}</h2>
        <p>
          {formatDate(period.from ?? null)} – {formatDate(period.to ?? null)} ·
      ...
      {overviewContent ? (
        <div className="report-v2-performance-view">
          <div className="report-v2-context">
            <span>
              Active Work: {overviewContent.work?.totals?.activeAtEnd ?? 0}
            </span>
            <span>
              Completed: {overviewContent.workflow?.completedDuring ?? 0}
            </span>
            <span>
              Need Review: {overviewContent.workflow?.waitingForApproval ?? 0}
            </span>
            <span>Overdue: {overviewContent.workflow?.overdue ?? 0}</span>
          </div>
          <div className="report-v2-performance-table-wrap" tabIndex={0}>
            <table className="report-v2-performance-table">
              <thead>
                <tr>
                  <th>Team</th>
                  <th>Active</th>
              ...
              <tbody>
                {overviewContent.teams.map((team) => (
                  <tr key={team.teamId}>
                    <td>{team.name}</td>
                    <td>{team.activeWork}</td>
                    <td>{team.inProgress}</td>
                    <td>{team.waitingForSales}</td>
                    <td>{team.waitingForApproval}</td>
                    <td>{team.overdueWork}</td>
                    <td>{team.completedDuring}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <table className="report-v2-performance-table">
            <thead>
              <tr>
                ...
                <th rowSpan={2}>Team</th>
                <th rowSpan={2}>Reference</th>
                {(performanceContent.workTypeGroups ?? []).map((group) => (
                  <th key={group.id} colSpan={3}>
                    {group.name}
                  </th>
                ))}
              ...
              <tr>
                {(performanceContent.workTypeGroups ?? []).flatMap((group) => [
                  <th key={`${group.id}-t`}>Tickets</th>,
                  <th key={`${group.id}-c`}>Completed</th>,
                  <th key={`${group.id}-p`}>Pending</th>,
                ])}
              </tr>
            </thead>
            <tbody>
              {(performanceContent.rows ?? []).map((row) => (
                <tr
                  key={`${row.date}:${row.operationalTeam?.id ?? row.orgUnit?.id ?? "unassigned"}`}
                >
                  <td>{formatDate(row.date)}</td>
                  <td>
                    {row.operationalTeam?.name ?? row.orgUnit?.name ?? "—"}
                  </td>
                  <td>
                    {(row.references ?? [])
                      .map((reference) => reference.display)
                  ...
                  {(performanceContent.workTypeGroups ?? []).flatMap(
                    (group) => {
                      const counts = row.workTypes?.[group.id] ?? {
                        tickets: 0,
                        completed: 0,
                      ...
                      return [
                        <td key={`${group.id}-t`}>{counts.tickets}</td>,
                        <td key={`${group.id}-c`}>{counts.completed}</td>,
                        <td key={`${group.id}-p`}>{counts.pending}</td>,
                      ];
                    },
                  )}
                </tr>
      ...
      {workRecordRows ? (
        <div className="report-v2-performance-table-wrap" tabIndex={0}>
          <table className="report-v2-performance-table">
            ...
            <tbody>
              {workRecordRows.map((row) => (
                <tr key={row.id}>
                  <td>{row.ticketNumber}</td>
                  <td>{row.workType?.name ?? "Work"}</td>
                  <td>
                    {row.executionTeams?.[0]?.name ??
                      row.primaryAssignee ??
                      "—"}
                  </td>
                  <td>{row.reference?.display ?? "—"}</td>
      ...
      {dutyContent ? (
        <div className="report-v2-performance-view">
          <div className="report-v2-context">
            <span>Scheduled: {dutyContent.dutySummary?.scheduled ?? 0}</span>
            <span>
              Employees: {dutyContent.dutySummary?.uniqueEmployees ?? 0}
            </span>
            <span>Leave: {dutyContent.dutySummary?.leaveDays ?? 0}</span>
            <span>Cancelled: {dutyContent.dutySummary?.cancelled ?? 0}</span>
          </div>
          <div className="report-v2-performance-table-wrap" tabIndex={0}>
            <table className="report-v2-performance-table">
              <thead>
              ...
              <tbody>
                {(dutyContent.sections?.duty?.rows ?? []).map((row) => (
                  <tr key={row.id}>
          ...
          {dutyContent.notice ? (
            <p className="report-v2-duty-note">{dutyContent.notice}</p>
          ) : null}
        </div>
      ) : null}
    </section>
...
function ReportPrintView({
  ...
  const title =
    view === "OVERVIEW"
      ? "Nepal Telecom Management Report"
      : view === "PERFORMANCE_REPORT"
        ? performancePeriodChoice === "TODAY"
          ? "Nepal Telecom Daily Work Performance Report"
          : performancePeriodChoice === "WEEK"
            ? "Nepal Telecom Weekly Work Performance Report"
            : performancePeriodChoice === "MONTH"
              ? "Nepal Telecom Monthly Work Performance Report"
              : "Nepal Telecom Work Performance Report"
        : view === "WORK_RECORDS"
          ? "Nepal Telecom Work Records Report"
          : "Nepal Telecom Duty Report";
  ...
  return (
    <section
      className="report-v2-print"
      aria-label={`${title} printable report`}
    >
                ...
                {overview.teams.length > 0 ? (
                  [...overview.teams]
                    .sort((left, right) => {
                      ...
                      if (right.waitingForApproval !== left.waitingForApproval)
                        return (
                          right.waitingForApproval - left.waitingForApproval
                        );
                      if (right.waitingForSales !== left.waitingForSales)
                        return right.waitingForSales - left.waitingForSales;
                      return right.activeWork - left.activeWork;
                  ...
                  <tr>
                    <td colSpan={7}>
                      No team performance rows for this scope and period.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {view === "PERFORMANCE_REPORT" &&
        ...
        (() => {
          ...
          const visibleGroups = performanceWorkTypeId
            ? section.workTypeGroups.filter(
                (group) => group.definitionId === performanceWorkTypeId,
              )
            : section.workTypeGroups;
          const showOverallTotal = visibleGroups.length > 1;
          return (
                    ...
                    <tr>
                      <td
                        colSpan={
                          ...
                          (showOverallTotal ? 3 : 0)
                        }
                      >
                        No operational Team work matched this period.
                      </td>
                    </tr>
                  )}
                </tbody>
                    ...
                    {visibleGroups.map((group) => (
                      <Fragment key={group.id}>
                        <td>
                          {
                            (
                              section.totals.workTypes[group.id] ?? {
                                tickets: 0,
                                completed: 0,
                                pending: 0,
                              }
          ...
          return (
            <section className="report-v2-print__section">
              <div className="report-v2-print__section-heading">
                ...
                <span>
                  {pagination
                    ? `${pagination.total} filtered records`
                    : `${rows.length} records`}
                </span>
              </div>
              <table className="report-v2-print__table is-records">
                ...
                <tbody>
                  {rows.length > 0 ? (
                    rows.map((row: WorkReportDrilldownWorkRow) => (
                      <tr key={row.id}>
                        <td>{row.ticketNumber}</td>
                        <td>{row.workType.name}</td>
            ...
            <>
              <section className="report-v2-print__section">
                ...
                <table className="report-v2-print__table is-kpis">
                  ...
                  <tbody>
                    <tr>
                      <td>{duty.dutySummary?.scheduled ?? 0}</td>
                      <td>{duty.dutySummary?.uniqueEmployees ?? 0}</td>
                      <td>{duty.dutySummary?.leaveDays ?? 0}</td>
                      <td>{duty.dutySummary?.cancelled ?? 0}</td>
                    </tr>
                  </tbody>
                </table>
              </section>
              <section className="report-v2-print__section">
                <div className="report-v2-print__section-heading">
                  ...
                  <span>
                    {pagination
                      ? `Page ${pagination.page} of ${pagination.totalPages} · ${pagination.total} filtered records`
                      : `${rows.length} records`}
                  </span>
                </div>
                <table className="report-v2-print__table is-duty">
                  ...
                  <tbody>
                    {rows.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan={8}>
                          No duty records found for the selected filters.
                        </td>
                      </tr>
                    )}
                  </tbody>
      ...
      <footer className="report-v2-print__footer">
        Generated by NT Message
        {view === "DUTY_REPORT"
          ? " · Planned duty is scheduling data, not attendance."
          : ""}
      </footer>
    </section>
  );
...
async function loadAllPrintableDrilldownRows(
  ...
  const firstSection =
    dataset === "WORK_RECORDS" ? first.sections.work : first.sections.duty;
...
export function WorkReportsPage() {
  ...
  const [overviewPeriodChoice, setOverviewPeriodChoice] =
    useState<PeriodChoice>("TODAY");
  const [overviewSummary, setOverviewSummary] =
    useState<WorkReportSummary | null>(null);
  const [overviewDraft, setOverviewDraft] = useState<WorkReportQuery>({
    from: today,
    to: today,
  });
  const [overviewApplied, setOverviewApplied] =
    useState<WorkReportQuery>(overviewDraft);
  const [overviewLoading, setOverviewLoading] = useState(true);

  const [performancePeriodChoice, setPerformancePeriodChoice] =
    useState<PeriodChoice>("TODAY");
  const [performanceDraft, setPerformanceDraft] = useState<WorkReportQuery>({
    from: today,
    to: today,
  });
  const [performanceApplied, setPerformanceApplied] =
    useState<WorkReportQuery>(performanceDraft);
  const [performanceReport, setPerformanceReport] =
    useState<WorkReportDrilldownResponse | null>(null);
  const [performanceLoading, setPerformanceLoading] = useState(false);

  const [recordsPeriodChoice, setRecordsPeriodChoice] =
    useState<PeriodChoice>("TODAY");
  ...
  const [recordsApplied, setRecordsApplied] =
    useState<WorkReportQuery>(recordsDraft);
  const [recordsReport, setRecordsReport] =
    useState<WorkReportDrilldownResponse | null>(null);
  const [recordsPage, setRecordsPage] = useState(1);
  const [recordsLoading, setRecordsLoading] = useState(false);

  const [dutyPeriodChoice, setDutyPeriodChoice] =
    useState<PeriodChoice>("TODAY");
  ...
  const [dutyReport, setDutyReport] =
    useState<WorkReportDrilldownResponse | null>(null);
  const [dutyPage, setDutyPage] = useState(1);
  const [dutyLoading, setDutyLoading] = useState(false);
  const [exporting, setExporting] = useState(false);
  const [preparingPrint, setPreparingPrint] = useState(false);
  const [printRecordsReport, setPrintRecordsReport] =
    useState<WorkReportDrilldownResponse | null>(null);
  const [printDutyReport, setPrintDutyReport] =
    useState<WorkReportDrilldownResponse | null>(null);
  const [error, setError] = useState("");
  const [notice, setNotice] = useState("");
  const [snapshots, setSnapshots] = useState<WorkReportSnapshotSummary[]>([]);
  const [selectedSnapshot, setSelectedSnapshot] =
    useState<WorkReportSavedSnapshot | null>(null);
  const [savingSnapshot, setSavingSnapshot] = useState(false);

  const refreshSnapshots = useCallback(async () => {
    if (!accessToken) return;
    try {
      setSnapshots(await listWorkReportSnapshots(accessToken));
    } catch {
      // Saved-report history is supplementary; the live report remains usable.
    }
  }, [accessToken]);
  ...
  const loadOverview = useCallback(async () => {
    ...
    try {
      setOverviewSummary(
        await getWorkReportSummary(accessToken, overviewApplied),
      );
    } catch (loadError) {
  ...
  const overviewTeams = useMemo(() => {
    ...
    return overviewSummary.teamOptions.filter((team) => {
      if (
        overviewDraft.departmentId &&
        team.departmentId !== overviewDraft.departmentId
      ) {
        return false;
      }
      if (
  ...
  const performanceTeams = useMemo(() => {
    ...
    return overviewSummary.teamOptions.filter((team) => {
      if (
        performanceDraft.departmentId &&
        team.departmentId !== performanceDraft.departmentId
      ) {
        return false;
      }
      if (
  ...
  const recordsTeams = useMemo(() => {
    ...
    return overviewSummary.teamOptions.filter((team) => {
      if (
        recordsDraft.departmentId &&
        team.departmentId !== recordsDraft.departmentId
      ) {
        return false;
      }
      if (
  ...
  const dutyVisibleTeams = useMemo(() => {
    if (!overviewSummary) return [];
    const selectedOrgUnit = dutyDraft.departmentId
      ? overviewSummary.departmentOptions.find(
          (item) => item.id === dutyDraft.departmentId,
        )
      : null;
    return overviewSummary.teamOptions.filter((team) => {
      if (
        dutyDraft.divisionId &&
        team.department.division.id !== dutyDraft.divisionId
      )
        return false;
      if (selectedOrgUnit) {
        const selectedPath = selectedOrgUnit.name;
        const teamPath = team.department.name;
        if (
          teamPath !== selectedPath &&
          !teamPath.startsWith(`${selectedPath} › `)
        )
          return false;
      }
      return true;
    });
  }, [dutyDraft.departmentId, dutyDraft.divisionId, overviewSummary]);
  ...
  const applyOverview = (event: FormEvent) => {
    ...
    setOverviewApplied({
      ...
      workTypeId: overviewDraft.workTypeId || undefined,
    });
  };
  ...
  const applyPerformance = (event: FormEvent) => {
    ...
    setPerformanceApplied({
      ...
      workTypeId: performanceDraft.workTypeId || undefined,
    });
  };
  ...
  const openWorkRecordsFromOverview = (options: {
    ...
    const next: WorkReportQuery = {
      ...
      workTypeId: overviewApplied.workTypeId || undefined,
  ...
  const applyRecords = (event: FormEvent) => {
    ...
    setRecordsApplied({
      ...
      workTypeId: recordsDraft.workTypeId || undefined,
  ...
  const submitDuty = (event: FormEvent) => {
    ...
    setDutyApplied({
      ...
      teamId: dutyDraft.teamId || undefined,
  ...
  const openSavedSnapshot = async (snapshotId: string) => {
    if (!accessToken) return;
    setError("");
    try {
      setSelectedSnapshot(await getWorkReportSnapshot(accessToken, snapshotId));
    } catch (loadError) {
      setError(getErrorMessage(loadError));
    }
  };
  ...
  const saveCurrentSnapshot = async () => {
    ...
    if (!accessToken || !ready || savingSnapshot) return;
    ...
    try {
      const saved = await saveWorkReportSnapshot(accessToken, dataset, query);

      setNotice(`Saved report: ${saved.name}`);
    ...
    try {
      if (view === "WORK_RECORDS") {
        setPrintRecordsReport(
          await loadAllPrintableDrilldownRows(
            accessToken,
            recordsApplied,
            "WORK_RECORDS",
          ),
  ...
  const showDivisionFilter = account?.role !== "EMPLOYEE";
  const showDepartmentFilter = account?.role !== "EMPLOYEE";
      ...
      <div className="performance-report-page__canvas" aria-busy={currentBusy}>
        <header className="performance-report-hero report-v2-hero">
          ...
          <div className="performance-report-hero__actions">
            <Link
              className="performance-report-action performance-report-action--back"
              to="/work"
            >
            ...
            <button
              ...
              disabled={!currentReportReady || savingSnapshot}
            >
              {savingSnapshot ? "Saving…" : "Save Report"}
            </button>
          </div>
        </header>
        ...
        {error && (
          <section
            className="performance-report-message report-v2-message is-error"
            role="alert"
          >
            <div>
        ...
        {notice && (
          <section
            className="performance-report-message report-v2-message is-success"
            role="status"
            aria-live="polite"
          >
            {notice}
        ...
        >
          <header className="report-v2-performance-panel__header">
            ...
            <h2>Saved Reports</h2>
            <p>
              Saved reports are immutable snapshots. Later Work Type,
              organization or Duty configuration changes do not rewrite them.
            </p>
          </header>
          {snapshots.length === 0 ? (
            <div className="report-v2-empty">No saved reports yet.</div>
          ) : (
            <div className="report-v2-performance-cards">
                ...
                >
                  <header>
                    <div>
                      ...
                      <small>
                        {formatDate(snapshot.periodFrom)} –{" "}
                        {formatDate(snapshot.periodTo)}
                      </small>
                    </div>
                  </header>
                  <dl>
                    <div>
                      <dt>Report</dt>
                      <dd>{snapshot.dataset.replaceAll("_", " ")}</dd>
                    </div>
                    <div>
                      <dt>Saved</dt>
                  ...
                  <button
                    type="button"
                    className="performance-report-action"
                    onClick={() => void openSavedSnapshot(snapshot.id)}
                  >
                    Open saved report
                  </button>
                </article>
        ...
        <nav
          className="performance-report-tabs report-v2-tabs"
        ...
        >
          ...
          <button
            type="button"
            aria-current={view === "PERFORMANCE_REPORT" ? "page" : undefined}
            className={view === "PERFORMANCE_REPORT" ? "is-active" : ""}
            onClick={() => setView("PERFORMANCE_REPORT")}
          >
            Performance Report
          </button>
          <button
            type="button"
            aria-current={view === "WORK_RECORDS" ? "page" : undefined}
            className={view === "WORK_RECORDS" ? "is-active" : ""}
            onClick={() => setView("WORK_RECORDS")}
          >
            Work Records
          </button>
          <button
            type="button"
            aria-current={view === "DUTY_REPORT" ? "page" : undefined}
            className={view === "DUTY_REPORT" ? "is-active" : ""}
            onClick={() => setView("DUTY_REPORT")}
          >
            Duty
          </button>
        </nav>
        ...
        {view === "OVERVIEW" ? (
          <>
            <form className="report-v2-filters" onSubmit={applyOverview}>
              <div className="report-v2-filter-heading">
                ...
                <div
                  className="report-v2-periods"
                  aria-label="Overview report period"
                >
                  {(
                    [
                      ["TODAY", "Today"],
                      ...
                      ["MONTH", "This Month"],
                      ["CUSTOM", "Custom"],
                    ] as Array<[PeriodChoice, string]>
                  ).map(([choice, label]) => (
                    <button
                      key={choice}
                      type="button"
                      aria-pressed={overviewPeriodChoice === choice}
                      className={
                        overviewPeriodChoice === choice ? "is-active" : ""
                      }
                      onClick={() => changeOverviewPeriod(choice)}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <div className="report-v2-filter-grid">
                ...
                {showDivisionFilter && (
                  <label>
                    ...
                    >
                      ...
                      {overviewDivisions.map((division) => (
                ...
                <label>
                  <span>Work type</span>
                  <select
                    value={overviewDraft.workTypeId ?? ""}
                    onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                      setOverviewDraft((current) => ({
                        ...current,
                  ...
                  >
                    ...
                    {(overviewSummary?.workTypeOptions ?? []).map(
                      (workType) => (
                ...
                <div className="report-v2-filter-actions">
                  ...
                  >
                    {overviewLoading ? "Updating…" : "Apply"}
                  </button>
                </div>
              </div>
            </form>
            ...
            {overviewSummary && (
              <div
                className="report-v2-context"
                aria-label="Current report context"
              >
                <span>
                  {formatDate(overviewSummary.period.from)} –{" "}
                  {formatDate(overviewSummary.period.to)}
                </span>
                <span>{overviewSummary.scope.label}</span>
                <span>
                  Generated {formatDateTime(overviewSummary.generatedAt)}
                </span>
              </div>
            )}
          ...
          <>
            <form
              className="report-v2-record-filters report-v2-performance-filters"
              onSubmit={applyPerformance}
            >
              <div className="report-v2-filter-heading">
                <div>
                ...
                <div
                  className="report-v2-periods"
                  aria-label="Performance report period"
                >
                  {(
                    [
                      ["TODAY", "Today"],
                    ...
                    <button
                      key={choice}
                      type="button"
                      aria-pressed={performancePeriodChoice === choice}
                      className={
                        performancePeriodChoice === choice ? "is-active" : ""
                      }
                      onClick={() => changePerformancePeriod(choice)}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <div className="report-v2-filter-grid report-v2-performance-filter-grid">
                ...
                <label>
                  <span>Work Type</span>
                  <select
                    value={performanceDraft.workTypeId ?? ""}
                    onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                      setPerformanceDraft((current) => ({
                        ...current,
                        workTypeId: event.target.value || undefined,
                  ...
                  >
                    ...
                    {(overviewSummary?.workTypeOptions ?? [])
                      .filter(
                        (workType) => workType.template !== "ADMINISTRATIVE",
                      )
                      .map((workType) => (
                        <option key={workType.id} value={workType.id}>
                          {workType.name}
                ...
                <div className="report-v2-filter-actions">
                  ...
                  >
                    {performanceLoading ? "Updating…" : "Apply"}
                  </button>
                </div>
              </div>
            </form>
            ...
            {performanceReport && (
              <section
                className="report-v2-performance-meta"
                aria-label="Current performance report context"
              >
                <div>
                  <span>Report Period</span>
                  <strong>
                    {formatDate(performanceReport.period.from)} –{" "}
                    {formatDate(performanceReport.period.to)}
                  </strong>
                </div>
                <div>
                  <span>Report For</span>
                  <strong>{performanceReport.scope.label}</strong>
                </div>
                <div>
                  <span>Rows Shown</span>
                  <strong>
                    {performanceReport.sections.performance?.rows.length ?? 0}
                  </strong>
                </div>
                <div>
                  <span>Generated</span>
                  <strong>
                    {formatDateTime(performanceReport.generatedAt)}
                  </strong>
                </div>
              </section>
            )}
            ...
            <PerformanceReportView
              ...
              workTypeId={performanceApplied.workTypeId}
            />
          </>
        ) : view === "WORK_RECORDS" ? (
          <>
            <form className="report-v2-record-filters" onSubmit={applyRecords}>
              <div className="report-v2-performance-period-row">
                ...
                <div
                  className="report-v2-periods"
                  aria-label="Work records period"
                >
                  {(
                    [
                      ["TODAY", "Today"],
                    ...
                    <button
                      key={choice}
                      type="button"
                      aria-pressed={recordsPeriodChoice === choice}
                      className={
                        recordsPeriodChoice === choice ? "is-active" : ""
                      }
                      onClick={() => changeRecordsPeriod(choice)}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <div className="report-v2-record-filter-grid">
                ...
                {showDepartmentFilter && (
                  <label>
                    ...
                    >
                      <option value="">All org units</option>
                      {recordsDepartments.map((department) => (
                        <option key={department.id} value={department.id}>
                          {department.name}
                        </option>
                      ))}
            ...
            {recordsApplied.workflowStage && (
              <div className="report-v2-active-filter" role="status">
                <span>
                  Work Records filter:{" "}
                  <strong>
                    {workflowStageFilterLabel(recordsApplied.workflowStage)}
                  </strong>
                </span>
                <button type="button" onClick={clearRecordsWorkflowStage}>
                  Clear
                </button>
              </div>
            )}
            ...
            {recordsReport && (
              <div
                className="report-v2-context"
                aria-label="Current work records context"
              >
                <span>
                  {formatDate(recordsReport.period.from)} –{" "}
                  {formatDate(recordsReport.period.to)}
                </span>
                <span>{recordsReport.scope.label}</span>
                <span>
                  Generated {formatDateTime(recordsReport.generatedAt)}
                </span>
              </div>
            )}
          ...
          <>
            <form
              className="report-v2-record-filters report-v2-duty-filters"
              onSubmit={submitDuty}
            >
              <div className="report-v2-filter-heading">
                <div>
                  ...
                  <p>
                    Search employee, ID, designation, shift, Org Unit, Team,
                    supervisor or location.
                  </p>
                </div>
                <div
                  className="report-v2-periods"
                  aria-label="Duty report period"
                >
                  {(
                    [
                      ["TODAY", "Today"],
                    ...
                    <button
                      key={choice}
                      type="button"
                      aria-pressed={dutyPeriodChoice === choice}
                      className={dutyPeriodChoice === choice ? "is-active" : ""}
                      onClick={() => changeDutyPeriod(choice)}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <div className="report-v2-duty-filter-grid">
                <label className="report-v2-duty-filter-search">
                  ...
                  <input
                    type="search"
                    placeholder="Employee, ID, team, supervisor or location"
                ...
                {showDepartmentFilter && (
                  <label>
                    ...
                    >
                      <option value="">All org units</option>
                      {dutyVisibleDepartments.map((department) => (
                        <option key={department.id} value={department.id}>
                          {department.name}
                        </option>
                      ))}
            ...
            {dutyReport && (
              <div
                className="report-v2-context"
                aria-label="Current duty report context"
              >
                <span>
                  {formatDate(dutyReport.period.from)} –{" "}
                  {formatDate(dutyReport.period.to)}
                </span>
        ...
        <ReportPrintView
          ...
          performanceWorkTypeId={performanceApplied.workTypeId}
