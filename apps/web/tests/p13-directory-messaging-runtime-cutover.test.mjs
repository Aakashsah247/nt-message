...
const directoryQueryDto = read(
  new URL("../../api/src/directory/dto/list-directory-query.dto.ts", import.meta.url),
);
...
const conversationsService = read(
  new URL("../../api/src/conversations/conversations.service.ts", import.meta.url),
);

test("P13-C Directory runtime no longer reads fixed hierarchy or management-position compatibility", () => {
  for (const source of [directoryService, directoryTypes, directoryList, directoryDetail]) {
...
test("P13-C Directory UI exposes account class, OrgUnit breadcrumb and leadership instead of legacy role/position", () => {
  assert.match(directoryTypes, /accountClass:\s*AccountClass \| null/);
  assert.match(directoryTypes, /orgUnitBreadcrumb/);
  assert.match(directoryTypes, /leadership:\s*DirectoryLeadershipAssignment\[\]/);
  assert.match(directoryList, /list\.table\.leadership/);
...
test("P13-C active messaging UI no longer branches on fixed cross-Division/Department request reasons", () => {
  ...
  const start = conversationsService.indexOf("private getMessageRequestReason(");
