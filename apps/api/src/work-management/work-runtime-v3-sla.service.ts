...
const TERMINAL_WORK_STATUSES = new Set<WorkRuntimeStatus>([
...
type TimingBucket =
  | 'participantWaitingMinutes'
  | 'dependencyOrActivationWaitingMinutes'
  | 'queueWaitingMinutes'
  | 'activeProcessingMinutes'
  | 'blockedMinutes'
  | 'approvalWaitingMinutes'
  | 'correctionWaitingMinutes';

@Injectable()
export class WorkRuntimeV3SlaService {
  constructor(
  ...
  async getWorkSlaSummary(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      return {
        ...
        slaBasis: work.workTypeVersion.slaBasis,
        dueAt: work.dueAt,
        isOverdue:
          !TERMINAL_WORK_STATUSES.has(work.runtimeStatus) &&
          work.dueAt.getTime() < now.getTime(),
        overdueByMinutes:
          !TERMINAL_WORK_STATUSES.has(work.runtimeStatus) &&
          work.dueAt.getTime() < now.getTime()
            ? this.minutesBetween(work.dueAt, now)
            : 0,
        stages: work.runtimeStages.map((stage) =>
          this.buildStageTiming(stage, now),
        ),
      };
    });
  }
  ...
  private buildStageTiming(
    ...
    const buckets: Record<TimingBucket, number> = {
      participantWaitingMinutes: 0,
      dependencyOrActivationWaitingMinutes: 0,
      queueWaitingMinutes: 0,
      activeProcessingMinutes: 0,
      blockedMinutes: 0,
      approvalWaitingMinutes: 0,
      correctionWaitingMinutes: 0,
    };
    const acceptedAt = stage.collaborationRequests[0]?.respondedAt ?? null;
    ...
    for (const event of stage.events) {
      ...
      currentStatus = event.toStageStatus;
    ...
    return {
      ...
      isOverdue:
        stage.dueAt !== null &&
        !TERMINAL_STAGE_STATUSES.has(stage.status) &&
        stage.dueAt.getTime() < now.getTime(),
      overdueByMinutes:
        stage.dueAt !== null &&
        !TERMINAL_STAGE_STATUSES.has(stage.status) &&
        stage.dueAt.getTime() < now.getTime()
          ? this.minutesBetween(stage.dueAt, now)
          : 0,
      ...buckets,
    };
  }

  private addTimingSegment(
    buckets: Record<TimingBucket, number>,
    status: WorkStageStatus,
    from: Date,
    to: Date,
    responsibleRule: WorkStageResponsibleOrgUnitRule,
    ...
    const key = this.bucketForStatus(status, responsibleRule, acceptedAt, from);
    if (key) {
      buckets[key] += this.minutesBetween(from, to);
    }
  }

  private bucketForStatus(
    status: WorkStageStatus,
    responsibleRule: WorkStageResponsibleOrgUnitRule,
    acceptedAt: Date | null,
    at: Date,
    ...
    switch (status) {
      case WorkStageStatus.PENDING:
        ...
        return 'dependencyOrActivationWaitingMinutes';
      case WorkStageStatus.READY:
        return 'queueWaitingMinutes';
      case WorkStageStatus.IN_PROGRESS:
        return 'activeProcessingMinutes';
      case WorkStageStatus.BLOCKED:
        return 'blockedMinutes';
      case WorkStageStatus.SUBMITTED:
        return 'approvalWaitingMinutes';
      case WorkStageStatus.RETURNED:
        return 'correctionWaitingMinutes';
      default:
        return null;
    }
  }
