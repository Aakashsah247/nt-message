...
export class EmployeesService {
  ...
  async transferEmployee(
    ...
    if (user.role !== AccountRole.SUPER_ADMIN) {
      throw new ForbiddenException(
        'Only the Super Admin can transfer an employee.',
      );
    }
    ...
    if (reason.length < 3) {
      throw new BadRequestException(
        'Transfer reason must contain at least 3 characters.',
      );
    }
    ...
    if (Number.isNaN(effectiveAt.getTime())) {
      throw new BadRequestException('Transfer effective date is invalid.');
    }
    ...
    const result = await this.prisma.$transaction(async (transaction) => {
      ...
      if (employee.account?.role === AccountRole.SUPER_ADMIN) {
        throw new ForbiddenException(
          'The Super Admin cannot be transferred through this process.',
        );
      }

      if (employee.employmentStatus !== EmploymentStatus.ACTIVE) {
        throw new ConflictException(
          'Only an actively employed user can be transferred.',
        );
      }

      if (employee.archivedAt) {
        throw new ConflictException(
          'An archived employee cannot be transferred.',
        );
      }
      ...
      if (
        ...
        throw new ConflictException(
          'The employee already belongs to the selected division and department.',
        );
      }
      ...
      const updatedEmployee = await transaction.employee.update({
        ...
        data: {
          ...
          // Keep the temporary legacy field synchronized.
          department: department.name,
      ...
      await transaction.employeeLifecycleAction.create({
        data: {
          ...
          newEmploymentStatus: EmploymentStatus.ACTIVE,
          ...
          metadata: {
            previousDivision: {
              ...
              code: employee.division?.code ?? null,

              name: employee.division?.name ?? null,
            },

            previousDepartment: {
              id: employee.departmentUnit?.id ?? null,

              code: employee.departmentUnit?.code ?? null,

              name: employee.departmentUnit?.name ?? null,
            },
